...
class ProductRestControllerTest {
  ...
  void findAllSuccessTest() throws Exception {
    ...
    result.andDo(print()).andExpect(status().isOk()).andExpect(handler().handlerType(ProductRestController.class))
        ...
        .andExpect(jsonPath("$.response").isArray()).andExpect(jsonPath("$.response.length()", is(3)))
        .andExpect(jsonPath("$.response[0].seq", is(3))).andExpect(jsonPath("$.response[0].name", is("Product C")))
        .andExpect(jsonPath("$.response[0].details", is("Very good product")))
        .andExpect(jsonPath("$.response[0].reviewCount").exists())
        .andExpect(jsonPath("$.response[0].reviewCount").isNumber()).andExpect(jsonPath("$.response[1].seq", is(2)))
        ...
        .andExpect(jsonPath("$.response[1].reviewCount").isNumber()).andExpect(jsonPath("$.response[2].seq", is(1)))
  ...
  void findByIdSuccessTest() throws Exception {
    for (int i = 1; i < 4; i++) {
      ResultActions result = mockMvc.perform(get("/api/products/" + i).accept(MediaType.APPLICATION_JSON));
      result.andDo(print()).andExpect(status().isOk()).andExpect(handler().handlerType(ProductRestController.class))
  ...
  void findByIdFailureTest() throws Exception {
    ResultActions result = mockMvc.perform(get("/api/products/100").accept(MediaType.APPLICATION_JSON));
    result.andDo(print()).andExpect(status().is4xxClientError())
        .andExpect(handler().handlerType(ProductRestController.class)).andExpect(handler().methodName("findById"))
