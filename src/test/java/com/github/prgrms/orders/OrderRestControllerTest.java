...
class OrderRestControllerTest {
  ...
  void findAllSuccessTest1() throws Exception {
    ResultActions result = mockMvc
        .perform(get("/api/orders").accept(MediaType.APPLICATION_JSON).param("offset", "0").param("size", "2"));
    result.andDo(print()).andExpect(status().isOk()).andExpect(handler().handlerType(OrderRestController.class))
        .andExpect(handler().methodName("findAll")).andExpect(jsonPath("$.success", is(true)))
        .andExpect(jsonPath("$.response").isArray()).andExpect(jsonPath("$.response.length()", is(2)))
  ...
  void findAllSuccessTest2() throws Exception {
    ResultActions result = mockMvc
        .perform(get("/api/orders").accept(MediaType.APPLICATION_JSON).param("offset", "2").param("size", "1"));
    result.andDo(print()).andExpect(status().isOk()).andExpect(handler().handlerType(OrderRestController.class))
        .andExpect(handler().methodName("findAll")).andExpect(jsonPath("$.success", is(true)))
        .andExpect(jsonPath("$.response").isArray()).andExpect(jsonPath("$.response.length()", is(1)))
  ...
  void findAllSuccessTest3() throws Exception {
    ResultActions result = mockMvc
        .perform(get("/api/orders").accept(MediaType.APPLICATION_JSON).param("offset", "-1").param("size", "10"));
    result.andDo(print()).andExpect(status().isOk()).andExpect(handler().handlerType(OrderRestController.class))
        .andExpect(handler().methodName("findAll")).andExpect(jsonPath("$.success", is(true)))
        .andExpect(jsonPath("$.response").isArray()).andExpect(jsonPath("$.response.length()", is(5)))
        .andExpect(jsonPath("$.response[0].seq", is(7)))
        .andExpect(jsonPath("$.response[0].productId", is(productIdForOrderId.get(7))))
        .andExpect(jsonPath("$.response[0].state", is(stateForOrderId.get(7))))
        .andExpect(jsonPath("$.response[1].seq", is(6)))
        ...
        .andExpect(jsonPath("$.response[1].state", is(stateForOrderId.get(6))))
        .andExpect(jsonPath("$.response[1].rejectMessage", is(rejectMessageForOrderId.get(6))))
        .andExpect(jsonPath("$.response[1].rejectedAt").exists()).andExpect(jsonPath("$.response[2].seq", is(5)))
        ...
        .andExpect(jsonPath("$.response[2].completedAt").exists()).andExpect(jsonPath("$.response[3].seq", is(4)))
        ...
        .andExpect(jsonPath("$.response[3].completedAt").exists()).andExpect(jsonPath("$.response[3].review").exists())
  ...
  void findAllSuccessTest4() throws Exception {
    ResultActions result = mockMvc.perform(get("/api/orders").accept(MediaType.APPLICATION_JSON));
    result.andDo(print()).andExpect(status().isOk()).andExpect(handler().handlerType(OrderRestController.class))
        .andExpect(handler().methodName("findAll")).andExpect(jsonPath("$.success", is(true)))
        .andExpect(jsonPath("$.response").isArray()).andExpect(jsonPath("$.response.length()", is(5)))
        .andExpect(jsonPath("$.response[0].seq", is(7)))
  ...
  void findAllFailureTest() throws Exception {
    ResultActions result = mockMvc.perform(get("/api/orders").accept(MediaType.APPLICATION_JSON)
        .header(jwtTokenConfigure.getHeader(), "Bearer " + randomAlphanumeric(60)).param("offset", "0")
        .param("size", "5"));
    result.andDo(print()).andExpect(status().is4xxClientError()).andExpect(jsonPath("$.success", is(false)))
        .andExpect(jsonPath("$.error").exists()).andExpect(jsonPath("$.error.status", is(401)))
  ...
  void findByIdSuccessTest() throws Exception {
    for (int i = 1; i < 8; i++) {
      ResultActions result = mockMvc.perform(get("/api/orders/" + i).accept(MediaType.APPLICATION_JSON));
      result.andDo(print()).andExpect(status().isOk()).andExpect(handler().handlerType(OrderRestController.class))
          .andExpect(handler().methodName("findById")).andExpect(jsonPath("$.success", is(true)))
  ...
  void findByIdFailureTest() throws Exception {
    ResultActions result = mockMvc.perform(get("/api/orders/1").accept(MediaType.APPLICATION_JSON)
  ...
  void acceptSuccessTest() throws Exception {
    ResultActions result = mockMvc.perform(patch("/api/orders/1/accept").accept(MediaType.APPLICATION_JSON));
    result.andDo(print()).andExpect(status().isOk()).andExpect(handler().handlerType(OrderRestController.class))
        .andExpect(handler().methodName("accept")).andExpect(jsonPath("$.success", is(true)))
        .andExpect(jsonPath("$.response", is(true)));
    result = mockMvc.perform(get("/api/orders/1").accept(MediaType.APPLICATION_JSON));
    result.andDo(print()).andExpect(status().isOk()).andExpect(handler().handlerType(OrderRestController.class))
        .andExpect(handler().methodName("findById")).andExpect(jsonPath("$.success", is(true)))
        .andExpect(jsonPath("$.response.seq", is(1))).andExpect(jsonPath("$.response.state", is("ACCEPTED")));
  }
  ...
  void acceptFailureTest() throws Exception {
    for (int i = 2; i < 7; i++) {
      ResultActions result = mockMvc.perform(patch("/api/orders/" + i + "/accept").accept(MediaType.APPLICATION_JSON));
      result.andDo(print()).andExpect(status().isOk()).andExpect(handler().handlerType(OrderRestController.class))
          .andExpect(handler().methodName("accept")).andExpect(jsonPath("$.success", is(true)))
          .andExpect(jsonPath("$.response", is(false)));
      result = mockMvc.perform(get("/api/orders/" + i).accept(MediaType.APPLICATION_JSON));
      result.andDo(print()).andExpect(status().isOk()).andExpect(handler().handlerType(OrderRestController.class))
          .andExpect(handler().methodName("findById")).andExpect(jsonPath("$.success", is(true)))
          .andExpect(jsonPath("$.response.seq", is(i)))
          .andExpect(jsonPath("$.response.state", is(stateForOrderId.get(i))));
  ...
  void shippingSuccessTest() throws Exception {
    ResultActions result = mockMvc.perform(patch("/api/orders/2/shipping").accept(MediaType.APPLICATION_JSON));
    result.andDo(print()).andExpect(status().isOk()).andExpect(handler().handlerType(OrderRestController.class))
        .andExpect(handler().methodName("shipping")).andExpect(jsonPath("$.success", is(true)))
        .andExpect(jsonPath("$.response", is(true)));
    result = mockMvc.perform(get("/api/orders/2").accept(MediaType.APPLICATION_JSON));
    result.andDo(print()).andExpect(status().isOk()).andExpect(handler().handlerType(OrderRestController.class))
        .andExpect(handler().methodName("findById")).andExpect(jsonPath("$.success", is(true)))
        .andExpect(jsonPath("$.response.seq", is(2))).andExpect(jsonPath("$.response.state", is("SHIPPING")));
  }
  ...
  void shippingFailureTest() throws Exception {
    for (int i = 3; i < 8; i++) {
      ResultActions result = mockMvc
          .perform(patch("/api/orders/" + i + "/shipping").accept(MediaType.APPLICATION_JSON));
      result.andDo(print()).andExpect(status().isOk()).andExpect(handler().handlerType(OrderRestController.class))
          .andExpect(handler().methodName("shipping")).andExpect(jsonPath("$.success", is(true)))
          .andExpect(jsonPath("$.response", is(false)));
      result = mockMvc.perform(get("/api/orders/" + i).accept(MediaType.APPLICATION_JSON));
  ...
  void completeSuccessTest() throws Exception {
    ResultActions result = mockMvc.perform(patch("/api/orders/3/complete").accept(MediaType.APPLICATION_JSON));
    result.andDo(print()).andExpect(status().isOk()).andExpect(handler().handlerType(OrderRestController.class))
        .andExpect(handler().methodName("complete")).andExpect(jsonPath("$.success", is(true)))
        .andExpect(jsonPath("$.response", is(true)));
    result = mockMvc.perform(get("/api/orders/3").accept(MediaType.APPLICATION_JSON));
    result.andDo(print()).andExpect(status().isOk()).andExpect(handler().handlerType(OrderRestController.class))
        .andExpect(handler().methodName("findById")).andExpect(jsonPath("$.success", is(true)))
        .andExpect(jsonPath("$.response.seq", is(3))).andExpect(jsonPath("$.response.state", is("COMPLETED")))
        .andExpect(jsonPath("$.response.completedAt").exists());
  }
  ...
  void completeFailureTest() throws Exception {
    for (int i = 4; i < 8; i++) {
      ResultActions result = mockMvc
          .perform(patch("/api/orders/" + i + "/complete").accept(MediaType.APPLICATION_JSON));
      result.andDo(print()).andExpect(status().isOk()).andExpect(handler().handlerType(OrderRestController.class))
          .andExpect(handler().methodName("complete")).andExpect(jsonPath("$.success", is(true)))
          .andExpect(jsonPath("$.response", is(false)));
      result = mockMvc.perform(get("/api/orders/" + i).accept(MediaType.APPLICATION_JSON));
  ...
  void rejectSuccessTest() throws Exception {
    ResultActions result = mockMvc.perform(patch("/api/orders/7/reject").contentType(MediaType.APPLICATION_JSON)
        .accept(MediaType.APPLICATION_JSON).content("{\"message\":\"this is reject message!\"}"));
    result.andDo(print()).andExpect(status().isOk()).andExpect(handler().handlerType(OrderRestController.class))
        .andExpect(handler().methodName("reject")).andExpect(jsonPath("$.success", is(true)))
        .andExpect(jsonPath("$.response", is(true)));
    result = mockMvc.perform(get("/api/orders/7").accept(MediaType.APPLICATION_JSON));
    result.andDo(print()).andExpect(status().isOk()).andExpect(handler().handlerType(OrderRestController.class))
        .andExpect(handler().methodName("findById")).andExpect(jsonPath("$.success", is(true)))
        .andExpect(jsonPath("$.response.seq", is(7))).andExpect(jsonPath("$.response.state", is("REJECTED")))
  ...
  void rejectFailureTest1() throws Exception {
    ResultActions result = mockMvc.perform(
        patch("/api/orders/7/reject").contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON));
    result.andDo(print()).andExpect(status().is4xxClientError())
        .andExpect(handler().handlerType(OrderRestController.class)).andExpect(handler().methodName("reject"))
        .andExpect(jsonPath("$.success", is(false))).andExpect(jsonPath("$.error").exists())
  ...
  void rejectFailureTest2() throws Exception {
    for (int i = 2; i < 7; i++) {
      ResultActions result = mockMvc
          .perform(patch("/api/orders/" + i + "/reject").contentType(MediaType.APPLICATION_JSON)
              .accept(MediaType.APPLICATION_JSON).content("{\"message\":\"this is reject message!\"}"));
      result.andDo(print()).andExpect(status().isOk()).andExpect(handler().handlerType(OrderRestController.class))
          .andExpect(handler().methodName("reject")).andExpect(jsonPath("$.success", is(true)))
          .andExpect(jsonPath("$.response", is(false)));
