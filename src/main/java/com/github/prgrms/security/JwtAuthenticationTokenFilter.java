		...
		if (SecurityContextHolder.getContext().getAuthentication() == null) {
			...
			if (authorizationToken != null) {
				try {
					...
					this.log.debug("Jwt parse result: {}", claims);

					Long userKey = claims.userKey;
					String name = claims.name;
					List<GrantedAuthority> authorities = this.obtainAuthorities(claims);

					if (nonNull(userKey) && isNotEmpty(name) && (authorities.size() > 0)) {
						JwtAuthenticationToken authentication = new JwtAuthenticationToken(
								new JwtAuthentication(userKey, name), null, authorities);
						authentication.setDetails(new WebAuthenticationDetailsSource().buildDetails(request));
						SecurityContextHolder.getContext().setAuthentication(authentication);
					}
				} catch (Exception e) {
					this.log.warn("Jwt processing failed: {}", e.getMessage());
				}
			}
		} else {
			this.log.debug("SecurityContextHolder not populated with security token, as it already contained: '{}'",
	...
	private String obtainAuthorizationToken(HttpServletRequest request) {
		...
		if (token != null) {
			if (this.log.isDebugEnabled()) {
				this.log.debug("Jwt authorization api detected: {}", token);
			}
			try {
